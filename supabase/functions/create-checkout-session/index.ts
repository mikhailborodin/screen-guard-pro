...
const requiredBillingInterval = (value: unknown): BillingInterval => {
  if (value !== "month" && value !== "year" && value !== "lifetime") {
    throw new Error("billingInterval must be month, year or lifetime");
  }
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const metadata = { feature, source, extensionId, billingInterval, activationTokenHash };
    const stripe = new Stripe(stripeSecretKey, { apiVersion: "2026-07-29.dahlia" });
    const session = await stripe.checkout.sessions.create({
      mode: isLifetime ? "payment" : "subscription",
      // The amount is server-owned; clients cannot change the lifetime price.
      line_items: isLifetime
        ? [{ price_data: {
            currency: "usd",
            unit_amount: 5900,
            product_data: { name: "Smart Auto Blur Pro — Lifetime", description: "Lifetime Pro access. One payment, no renewal." },
          }, quantity: 1 }]
      ...
      ...(isLifetime ? { adaptive_pricing: { enabled: false } } : {}),
      integration_identifier: `privacyblur_web_${randomLetters(8)}`,
      metadata,
      ...(isLifetime ? { payment_intent_data: { metadata } } : { subscription_data: { metadata } }),
    });
