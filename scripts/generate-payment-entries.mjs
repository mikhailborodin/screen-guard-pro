...
// Keep the three payment entrypoints that are already published on GitHub Pages.
// They need real index.html files: GitHub Pages does not serve the SPA shell for these routes.
const paymentPaths = ["/paywall/", "/payment-success/", "/payment-cancelled/"];
const sharedStyles = `
...
const renderPaymentPage = (path) => `<!doctype html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <meta name="robots" content="noindex,nofollow" />
    <title>Screen Privacy Blur</title>
    <meta name="description" content="Screen Privacy Blur application page." />
    <link rel="canonical" href="https://privacyblur.co${path}" />
    <link rel="icon" type="image/png" href="/app-icon.png" sizes="128x128" />
    <link rel="apple-touch-icon" href="/app-icon.png" />
    <link rel="icon" href="/favicon.ico" sizes="any" />
    <meta property="og:title" content="Screen Privacy Blur" />
    <meta property="og:description" content="Screen Privacy Blur application page." />
    <meta property="og:type" content="website" />
    <meta property="og:url" content="https://privacyblur.co${path}" />
    <meta property="og:image" content="https://privacyblur.co/social-preview.png" />
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:title" content="Screen Privacy Blur" />
    <meta name="twitter:description" content="Screen Privacy Blur application page." />
    <meta name="twitter:image" content="https://privacyblur.co/social-preview.png" />
    <style>${sharedStyles}</style>
    ${""}
    <script async src="https://www.googletagmanager.com/gtag/js?id=G-PE695ZZE0F"></script>
    <script>window.dataLayer = window.dataLayer || []; window.gtag = function gtag(){ window.dataLayer.push(arguments); }; window.gtag("js", new Date()); window.gtag("config", "G-PE695ZZE0F");</script>
    ${""}
    ${appAssets}
  </head>
  <body><div id="root"><main class="shell"><p>Loading Screen Privacy Blur…</p></main></div></body>
</html>`;

await Promise.all(paymentPaths.map(async (path) => {
  const target = join(distDir, path.slice(1), "index.html");
  await mkdir(dirname(target), { recursive: true });
  await writeFile(target, renderPaymentPage(path));
}));
